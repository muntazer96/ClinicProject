...
using Clinic_Booking.Services.DoctorRequestServices;
using Clinic_Booking.Services.TelegramAlertServices;
...
using Clinic_Booking.Services.DatabaseBackupServices;
...
builder.Services.AddScoped<IDoctorRequestServices, DoctorRequestServices>();
...
builder.Services.AddRateLimiter(options =>
{
    ...
    options.AddPolicy("DoctorRequestSubmit", httpContext =>
        RateLimitPartition.GetFixedWindowLimiter(
            httpContext.Connection.RemoteIpAddress?.ToString() ?? "unknown",
            _ => new FixedWindowRateLimiterOptions
            {
    ...
    options.AddPolicy("PublicRead", httpContext =>
        RateLimitPartition.GetFixedWindowLimiter(
            httpContext.Connection.RemoteIpAddress?.ToString() ?? "unknown",
            _ => new FixedWindowRateLimiterOptions
            {
