...
namespace Clinic_Booking.Services.AppointmentServices
{
    public class AppointmentServices : IAppointmentServices
    {
        ...
        public async Task<ActionResult<PaginationDto.PageResult<BookingDetailsDto>>> GetMineForDoctorAsync(SearchAppointmentDto form, int page = 1, int pageSize = 10)
        {
            if (page <= 0 || pageSize <= 0)
            {
                return new BadRequestObjectResult(new ResponseDto<string>
            ...
            var query = ProjectBookingDetails(_context.Appointments
                .Where(appointment =>
            ...
            return new OkObjectResult(new PaginationDto.PageResult<BookingDetailsDto>(
                appointments, totalItems, totalPages, page, pageSize));
        }
        ...
        public async Task<IActionResult> GetDoctorStatisticsAsync()
        {
            ...
            var todayStart = BusinessClock.TodayDateOnly().ToDateTime(TimeOnly.MinValue);

            var counts = await _context.Appointments
                .Where(a =>
                    !a.IsDeleted &&
                    a.DoctorId == doctorId &&
                    a.AppointmentDate.Date == todayStart.Date)
                .GroupBy(a => a.Status)
                .Select(g => new { Status = g.Key, Count = g.Count() })
                .ToListAsync();

            var pending = counts.FirstOrDefault(c => c.Status == AppointmentStatus.Pending)?.Count ?? 0;
            var confirmed = counts.FirstOrDefault(c => c.Status == AppointmentStatus.Confirmed)?.Count ?? 0;
            var cancelled = counts.FirstOrDefault(c => c.Status == AppointmentStatus.Cancelled)?.Count ?? 0;
            var completed = counts.FirstOrDefault(c => c.Status == AppointmentStatus.Completed)?.Count ?? 0;
            var totalToday = pending + confirmed + cancelled + completed;

            return new OkObjectResult(new ResponseDto<DoctorAppointmentStatisticsDto>
            {
                Status = "Success",
                Code = 200,
                Message = "تم جلب إحصائيات الحجوزات بنجاح.",
                Data = new DoctorAppointmentStatisticsDto
                {
        ...
        public async Task<IActionResult> GetNextUpcomingAsync()
        {
            var userId = GetAuthenticatedUserId();
            if (!userId.HasValue)
            {
            ...
            return new OkObjectResult(new ResponseDto<BookingDetailsDto>
            {
                Status = "Success",
                Code = 200,
                Message = "تم جلب الحجز القادم بنجاح.",
                Data = appointment
            });
        }
        ...
        public async Task<ActionResult<PaginationDto.PageResult<BookingDetailsDto>>> GetMyAppointmentsAsync(DateOnly? fromDate, DateOnly? toDate, int page = 1, int pageSize = 10)
        {
            if (page <= 0 || pageSize <= 0)
            {
                return new BadRequestObjectResult(new ResponseDto<string>
            ...
            var baseQuery = _context.Appointments.Where(a => !a.IsDeleted && a.UserId == userId);

            if (fromDate.HasValue)
            {
                var from = fromDate.Value.ToDateTime(TimeOnly.MinValue);
                baseQuery = baseQuery.Where(a => a.AppointmentDate.Date >= from);
            }

            if (toDate.HasValue)
            {
                var to = toDate.Value.ToDateTime(TimeOnly.MinValue);
                baseQuery = baseQuery.Where(a => a.AppointmentDate.Date <= to);
            }

            var projectedQuery = ProjectBookingDetails(baseQuery);

            var totalItems = await projectedQuery.CountAsync();
            var totalPages = (int)Math.Ceiling(totalItems / (double)pageSize);

            var appointments = await projectedQuery
                .OrderByDescending(a => a.AppointmentDate)
                .ThenBy(a => a.QueueNumber)
                .Skip((page - 1) * pageSize)
                .Take(pageSize)
